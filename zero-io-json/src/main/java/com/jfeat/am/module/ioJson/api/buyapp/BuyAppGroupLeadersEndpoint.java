...
@Api("mock data测试用")
@RequestMapping("/api/u/v1/groupLeaders")
public class BuyAppGroupLeadersEndpoint {
...
//        return SuccessTip.create(mockJsonService.readJsonFile(id));
    ...
    public JSONObject getGroupMembers(@PathVariable(value = "id") Long id){
        QueryWrapper<FrontPage> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq(FrontPage.PAGE_ID, id);
        FrontPage frontPage = frontPageMapper.selectOne(queryWrapper);
        String content = frontPage.getContent();
    ...
    @PutMapping("/{id}/groupMembers/{index}")
    public Tip updateGroupMembers(@PathVariable("id") Long id, @PathVariable("index") Integer index,@RequestBody JSONObject entity) {
        QueryWrapper<FrontPage> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq(FrontPage.PAGE_ID, id);
        FrontPage frontPage = frontPageMapper.selectOne(queryWrapper);
        String content = frontPage.getContent();
    ...
    @GetMapping("/{groupLeaderId}/groupPurchases/{id}")
    @ApiOperation(value = "查看 Json")
    public Tip getJson(@PathVariable("groupLeaderId") Long groupLeaderId,@PathVariable(value = "id") Long id) {
        QueryWrapper<FrontPage> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq(FrontPage.PAGE_ID, id);
        FrontPage frontPage = frontPageMapper.selectOne(queryWrapper);
        String content = frontPage.getContent();
